...
Write method prefixMerge(), which tainitiales 3 array parameters.  The
...
public class ClientTester {
    ...
    public static void prefixMerge(Client [] list1, Client [] list2, Client[] result){
        int listCounterUno = 0;
        int listCounterDos = 0;

         for (int initial = 0; initial < result.length; initial++) {
            if (listCounterUno < list1.length && listCounterDos < list2.length) {
                if (list1[listCounterUno].compareClient(list2[listCounterDos]) < 0) {
                    result[initial] = list1[listCounterUno];
                    listCounterUno++;
                } 
                else if (list1[listCounterUno].compareClient(list2[listCounterDos]) > 0) {
                    result[initial] = list2[listCounterDos];
                    listCounterDos++;
                } 
                else {
                    result[initial] = list1[listCounterUno];
            ...
            else if (listCounterUno < list1.length) {
                result[initial] = list1[listCounterUno];
                listCounterUno++;
            }
            else {
